...
public class CombatV extends GameScreen{
	...
	Pkm pkm=PokemonCore.Jtest.getTeam()[0];
	...
	public float cout=-1;
	public CombatV(Combat c,PokemonCore PokemonCore,MapScreen ms){
		state=-1;
		this.getStage().clear();
		this.PokemonCore=PokemonCore;
		dbox=new DialogBox(new Vector2(640,100),true);
		dbox.setMessage("Dresseur veut se battre");
		for(int i=0;i<PokemonCore.Jtest.getTeam().length;i++){
			if(PokemonCore.Jtest.getTeam()[i].get(2)!=0){
				pkm=PokemonCore.Jtest.getTeam()[i];
		...
		listener=new CombatListener(PokemonCore, this, c,ms);
